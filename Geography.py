...
class Path():
	...
	def __init__(self, city1, city2, length):
		...
		city1.addPath(self)
		city2.addPath(self)
		city1.addReachable(city2)
		city2.addReachable(city1)
...
class City(object):
	...
	def __init__(self, pos, isCap = False, void = False):
		...
		self.reachable = [] # Erreichbare Städte
	...
	def addReachable(self, city):
		addsafe(self.reachable, city)
	...
	def wayToExists(self, city):
		...
		try:
			...
			for n, i in enumerate(c):
				if i[1] in self.reachable:
					# Das funktioniert nur, wenn die Distanzen zwischen den Städten alle euklidisch sind!
					if i[1] == city:
			...
			while c:
				...
				for n, i in enumerate(c):
					if i[1] in destination.reachable:
						olddist = c[n][0]
						newdist = distance + destination.eucDist(i[1].getPos())
						if newdist < olddist:
							c[n] = (newdist, i[1], shortest[1])
			return True
	...
	def eucDist(self, point):
		return ((self.pos[0] - point[0]) ** 2 + (self.pos[1] - point[1]) ** 2) ** 0.5
